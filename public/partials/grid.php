...
<div id="<?php echo $base_location ?>-hours" class="hours-display<?php echo $hours_display ?>">
    ...
    <?php if (count($occupancy_data) > 0 && !array_key_exists('error', $occupancy_data)): ?>
        <?php $location_occupancy = $this->spacy_client->getLocationOccupancy($location, $occupancy_data) ?>
        <?php if (count($location_occupancy) > 0): ?>
            <div class="spacy <?php echo $location_occupancy['how_busy_class'] ?>">
                (<?php echo $location_occupancy['how_busy'] ?>)
            </div>
        <?php endif ?>
    <?php endif; ?>
</div>